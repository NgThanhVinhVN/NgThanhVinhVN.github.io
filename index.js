...
(function setClock() {
    ...
    update('#timezone-diff', tzDiff === 0 ? 'same time' : (tzDiff > 0 ? `${formatTimezone(tzDiff)} ahead` : `${formatTimezone(-tzDiff)} behind`));
    update('#utc-offset', ` / UTC ${(tzOff >= 0 ? '+' : '')}${Math.floor(tzOff)}:${(tzOff % 1 * 60).toString().padStart(2, '0')}`);
    setTimeout(setClock, 1000 - time % 1000);
    function formatTimezone(tzDiff) {
        if (tzDiff < 0) return `-${formatTimezone(-tzDiff)}`;
...
function update(selector, value) {
    ...
    if (value.startsWith('rotate')) element.style.transform = value;
    else if (value.match(/^#[a-f0-9]{3}$/)) element.style.backgroundColor = value;
    else if (value.startsWith('--bg')) element.style.setProperty(value.split(':')[0], value.split(' ')[1]);
    else if (value === '' && selector.includes('-image')) element.removeAttribute('style');
...
(function lanyard() {
    ...
    ws.addEventListener('message', async ({ data }) => {
        ...
        if (!activities || !activities.length || d.discord_status === 'offline') {
        ...
        prev.timestamp = a.timestamps?.end ? a.timestamps.end : a.timestamps?.start;
...
function setTimestamp(v) {
    ...
    const diff = Math.abs(v - Date.now());
    const hour = Math.floor(diff / 1000 / 60 / 60);
    const minute = Math.floor(diff / 1000 / 60) % 60;
    const second = Math.floor(diff / 1000) % 60;
    update('.timestamp', `${hour ? `${hour.toString().padStart(2, '0')}:` : ''}${minute.toString().padStart(2, '0')}:${second.toString().padStart(2, '0')} ${v > Date.now() ? 'left' : 'elapsed'}`);
}
